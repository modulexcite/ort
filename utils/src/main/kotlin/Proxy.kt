...
class OrtProxySelector(val fallback: ProxySelector? = null) : ProxySelector() {
    companion object {
        fun install(): OrtProxySelector {
            val default = getDefault()
            return if (default is OrtProxySelector) {
                default
            } else {
                OrtProxySelector(default).also { setDefault(it) }
            }
        }
        ...
        fun uninstall(): ProxySelector? {
            val default = getDefault()
            return if (default is OrtProxySelector) {
                default.fallback.also { setDefault(it) }
            } else {
                default
            }
        }
    ...
    private val proxyOrigins = mutableMapOf(
        "env" to mapOf(
...
val ortProxySelector = OrtProxySelector.install()
